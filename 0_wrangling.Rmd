---
title: "Join health, social/pop and environmental data for the Amazon project"
...
knitr::kable(master_health[sample(nrow(master_health), 30),], booktabs = TRUE) %>%
...
knitr::kable(pop_data[sample(nrow(pop_data), 30),], booktabs = TRUE) %>%
kable_styling(latex_options = "scale_down") %>%
...
knitr::kable(idh_data[sample(nrow(idh_data), 30),], booktabs = TRUE) %>%
kable_styling(latex_options = "scale_down") %>%
...
knitr::kable(combined_data[sample(nrow(combined_data), 30), c(1:3, 25:dim(combined_data)[2])], 
             booktabs = TRUE) %>%
kable_styling(latex_options = "scale_down") %>%
  column_spec(1:15, width = "auto")
```
...
for (country in unique_countries) {
  # Filtrar o dataset pelo país atual
...
```{r}
# Criando a descrição do metadata
metadata <- tribble(
